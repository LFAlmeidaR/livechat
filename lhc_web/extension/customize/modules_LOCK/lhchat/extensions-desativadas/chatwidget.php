...
if (isset($_POST['StartChat']) && $disabled_department === false)
{
   ...
   {   	
        ...
        $chat->lsync = time();
   		...
   		if($chat->city == ""){
			$cidade = (int)$chat->phone; 
            $defaults = array(
                CURLOPT_URL => 'https://ddd.pricez.com.br/ddds/'.$cidade.'.json',
			...
			if(!($respE===null)){
				$defaults2 = array(
					CURLOPT_URL => 'https://ddd.pricez.com.br/estados/'.strtolower($respE['payload']['0']['estado']).'.json',
					CURLOPT_RETURNTRANSFER => true,
				);
				$ch2 = curl_init();
				curl_setopt_array($ch2, $defaults2);
				$respC = json_decode(curl_exec($ch2), true);
				curl_close($ch2);
				foreach($respC['payload'] as $cidades){
					if(isset($cidades['capital']) && $cidades['capital']===true){
						$capital = $cidades['cidade'];
						break;
					}
				}
				$chat->country_code = "br";
				$chat->country_name = "Brazil";
				$chat->city = $capital.', '.$respE['payload']['0']['estado'];
			} else {
				$chat->city = "DDD Invalido!";
			}			   
		}
